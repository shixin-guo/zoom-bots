...
console.log("🚀 ~ file: subscriptionPlans.ts:8 ~ data: Model:", data)
...
console.log(data.plans[tierConstants.TIER_STARTUP_PLAN_ID]);
console.log('~~~~~~~~~~~~~~~');
...
export function getPlan(planId: PlanName) {
  if (planId === tierConstants.TIER_FREE_PLAN_ID) {
    ...
    return {
      ...
      interval: (businessPlanTier?.interval) || "monthly",
      name: (businessPlanTier?.title ) || "Free",
...
export const startupPlan: TierPlan = {
  ...
  promoted: true,
  name: (startupPlanTier?.title) || "Startup",
  base: getBase(startupPlanTier),
